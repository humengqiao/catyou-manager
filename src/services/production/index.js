      ...
      return queryCount
        ...
        .then(result => {
          const data = {
            list: list.map(item => ({
							...item,
							highlightProductionName: !keyword ? item.productionName : item.productionName.replace(new RegExp('(' + keyword + '+' + ')', 'g'), `<em style="color: ${highlightColor};font-size: 14px;text-decoration: underline;font-weight: bold;">$1</em>`)
						})),
            pagination: {
              total: result,
              pageSize,
